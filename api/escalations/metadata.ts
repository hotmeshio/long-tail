...
export async function findByMetadata(
  ...
  try {
    ...
    // RBAC: scope to the caller's read access. read_all roles see every match;
    // read_self roles see only matches assigned to them. (Bounded result set —
    // exact metadata match — so the visibility filter applies in-process.)
    const scope = await getEscalationReadScope(auth.userId);
    if (!scope.global) {
      const allSet = new Set(scope.allRoles);
      const selfSet = new Set(scope.selfRoles);
      result.escalations = result.escalations.filter(
        (e) => allSet.has(e.role) || (selfSet.has(e.role) && e.assigned_to === auth.userId),
      );
