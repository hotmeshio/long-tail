...
export async function findCompiledWorkflows(
  ...
  loggerRegistry.debug(`[${logPrefix}:findCompiledWorkflows] keywords: [${keywords.join(',')}]`);
  ...
  try {
    ...
    loggerRegistry.info(`[${logPrefix}:findCompiledWorkflows] ${workflows.length} candidate(s) found`);
  } catch (err: any) {
    loggerRegistry.warn(`[${logPrefix}:findCompiledWorkflows] discovery failed: ${err.message}`);
...
export async function evaluateWorkflowMatch(
  ...
  loggerRegistry.debug(`[${logPrefix}:evaluateWorkflowMatch] ${candidates.length} candidate(s)`);
  ...
  try {
    ...
    if (result.match && result.confidence >= 0.7) {
      loggerRegistry.info(`[${logPrefix}:evaluateWorkflowMatch] MATCHED: ${result.workflow_name} (confidence: ${result.confidence})`);
      return { matched: true, workflowName: result.workflow_name, confidence: result.confidence };
    }
    loggerRegistry.info(`[${logPrefix}:evaluateWorkflowMatch] no match (confidence: ${result.confidence || 0})`);
    return { matched: false, workflowName: null, confidence: result.confidence || 0 };
  } catch (err: any) {
    loggerRegistry.warn(`[${logPrefix}:evaluateWorkflowMatch] error: ${err.message}`);
    return { matched: false, workflowName: null, confidence: 0 };
  }
}
