...
function LifecycleBar({ e }: { e: Parsed }) {
  ...
  return (
    <div className="mt-2 h-0.5 rounded-full overflow-hidden flex bg-slate-100">
...
function GapZone({ gapMs, heightPx }: { gapMs: number; heightPx: number }) {
  return (
    <div className="relative flex items-center justify-center" style={{ height: heightPx }}>
      {gapMs >= 60_000 && (
        <span className="text-[9px] text-slate-300 font-mono bg-white px-2 relative z-10">
          {fmtMs(gapMs)}
        </span>
      )}
    </div>
...
function TimelineCard({
  ...
  const color   = STATUS_COLOR[e.displayStatus] ?? '#94a3b8';
  ...
  return (
    <div
      className="cursor-pointer max-w-[460px] min-w-[260px] w-full bg-white rounded-lg shadow-sm border border-slate-200/60 px-3.5 pt-2.5 pb-2 hover:shadow-md hover:border-slate-300/60 transition-all duration-150"
    ...
    >
      ...
      <div className="flex items-baseline gap-2 mb-1.5">
        ...
        <span className="text-[12px] font-semibold text-slate-800 truncate" title={e.type}>
          {e.type}
          {e.subtype && e.subtype !== e.type && e.subtype !== '' && (
            <span className="font-normal text-slate-500"> · {e.subtype}</span>
          )}
        </span>
      </div>
      ...
      {shown.length > 0 && (
        <div className="border-t border-slate-100 pt-1 space-y-px">
          {shown.map(([k, v]) => {
            const sv          = String(v);
            ...
            return (
              ...
              >
                <span
                  className={`shrink-0 w-[60px] text-[9px] font-mono uppercase tracking-wide truncate ${
                    isHighlight ? 'text-accent' : 'text-slate-400'
                  }`}
                  title={k}
                >
                  {k}
                ...
                <span
                  className={`flex-1 min-w-0 text-[11px] font-medium truncate ${
                    isHighlight ? 'text-slate-800' : 'text-slate-600'
                  }`}
                  title={sv}
                >
                  {sv}
          ...
          {hiddenCount > 0 && (
            <button
              className="flex items-center gap-0.5 text-[9px] text-slate-400 hover:text-accent transition-colors mt-0.5 -mx-1 px-1 py-px"
  ...
  return (
    <div className="mt-4 pb-24">
      ...
      <div className="relative">
        <div className="absolute left-1/2 -translate-x-px inset-y-0 w-px bg-slate-200/80" />
          ...
          return (
            <div key={e.id}>
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1 flex justify-end items-center">
                  {isLeft ? (
                    ...
                    <span className="text-[10px] text-slate-400 font-mono tabular-nums pr-3">
                      {timing}
                    </span>
                  )}
                </div>
                ...
                {/* Dot */}
                <div
                  className="relative z-10 flex-shrink-0 w-3.5 h-3.5 rounded-full border-2 border-white cursor-pointer transition-all duration-150 hover:scale-[1.5]"
                  style={{ backgroundColor: color, boxShadow: `0 0 0 3px ${color}28, 0 1px 3px ${color}30` }}
                  onClick={() => onRowClick?.(e)}
                />
              ...
              <div className="flex justify-center relative z-10 mt-0.5">
                <span className="text-[9px] text-slate-400 font-mono tabular-nums bg-white px-1.5">
