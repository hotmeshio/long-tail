...
function BarcodePreview({ value }: { value: string }) {
  ...
  return (
    <div>
      <span className="block text-xs uppercase tracking-wide text-text-tertiary mb-1">Barcode</span>
      {invalid ? (
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border">
        ...
        <button type="button" onClick={onClose} className="text-text-tertiary hover:text-text-primary" title="Close">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        <form onSubmit={submitManual} className="space-y-2">
          <label className="block text-xs uppercase tracking-wide text-text-tertiary">
            Enter a code
          </label>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm rounded bg-accent text-text-inverse disabled:opacity-50"
            >
              Go
            </button>
          </div>
        ...
        <div className="space-y-2">
          <button
            ...
            className="text-xs uppercase tracking-wide text-text-tertiary hover:text-text-primary"
          >
            ...
            <div className="space-y-3">
              ...
              {diagnosticsOn && (
                <div className="border border-surface-border rounded">
                  <div className="px-2 py-1.5 text-2xs uppercase tracking-wide text-text-tertiary border-b border-surface-border">
                    Keydown trace — scan now, newest last
                  </div>
                  <div className="max-h-64 overflow-y-auto font-mono text-2xs leading-5 px-2 py-1">
                    {diagnostics.length === 0 && (
                      <div className="text-text-tertiary">Waiting for keys…</div>
                    )}
                    {diagnostics.map((d) => (
                      <div key={d.seq} className="whitespace-nowrap overflow-hidden text-ellipsis" title={`buffer: ${d.buffer}`}>
                        <span className="text-text-tertiary">{String(d.deltaMs).padStart(4)}ms </span>
                        <span className="text">{JSON.stringify(d.key)}</span>
                        {d.code && <span className="text-text-tertiary"> ({d.code})</span>}
                        <span className="text-text-tertiary"> — {d.note}</span>
                      </div>
                    ))}
                  </div>
                  <div className="px-2 py-1.5 text-2xs text-text-tertiary border-t border-surface-border">
