        ...
        <div className="flex items-center gap-3 ml-auto">
          {activeCategories.map((cat) => (
            <div key={cat} className="flex items-center gap-1">
              <span className={`w-2.5 h-2.5 rounded-full dot-ring ${outline ? `border ${CATEGORY_COLORS[cat]?.outline ?? 'border-text-tertiary'}` : CATEGORY_COLORS[cat]?.bar ?? 'bg-text-tertiary'}`} />
          ...
          {spans.some((s) => s.pending) && (
            <div className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-full dot-ring bg-status-warning animate-pulse" />
          ...
          <div key={`${lane.category}:${lane.name}`}>
            ...
            <div className="flex items-center border-b border-surface-border">
              <div className="w-56 shrink-0 py-3 pr-4 flex items-center gap-2">
                <span className={`w-2.5 h-2.5 rounded-full dot-ring shrink-0 ${catColor?.bar ?? 'bg-text-tertiary'}`} />
                <p
