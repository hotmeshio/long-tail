 ...
 * the host escalation's context. Isomorphic: the dashboard renders embeds from
 * it, and the API layer enforces the submit gate from the identical mapping.
 *
 ...
 *
 * When `assigned: "me"` and no viewer id is supplied (a surface with no
 * resolving user, e.g. an MCP resolve), the scope cannot be evaluated —
 * `enabled` is false rather than silently widening to another scope.
 */
